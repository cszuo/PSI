...
signs = []
for titem in SIGNS:
    (servicename,pattern)=titem.split('|',1)
    sign=(servicename,re.compile(pattern))
    signs.append(sign)   

def matchbanner(banner, target=None):
    for item in signs:
        if target!= None :
            if target==item[0]:
                if item[1].search(banner)!=None:
                    return item[0]
        else:
            if item[1].search(banner)!=None:
                return item[0]
...
def ishttps(ip, port):
    ...
    try:
        for probe in PROBES[1:3]:
            ...
            #print probe
            sock.send(probe);
            with Timeout(timeout_receive):
                resp = sock.recv(256)
            #print resp
            service=matchbanner(resp,target='http')
            if service != 'Unknown':
                sock.close()
                break
            sock.close()
...
def recogn(ip, port, debug = False, callback=None):
    service = None
    if debug: print ip, port
    try:
        fsock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        with Timeout(timeout_connect):
            fsock.connect((ip, int(port)))
        with Timeout(timeout_receive):
            resp = fsock.recv(256)
        service=matchbanner(resp)
    except:
        #if debug: print e
        for probe in PROBES:
            try:
